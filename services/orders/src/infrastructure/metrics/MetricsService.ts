...
export class MetricsService {
   ...
   */
  recordOrderCreated(quantity: number): void {
    this.increment('orders_created_total');
    this.increment('order_items_created_total', {}, quantity);
  }

  recordOrderCompleted(preparationTimeSeconds: number): void {
    this.increment('orders_completed_total');
    this.observe('order_preparation_time_seconds', preparationTimeSeconds);
  }

  recordOrderFailed(reason: string): void {
    this.increment('orders_failed_total', { reason });
  }

  recordOrderCancelled(): void {
    this.increment('orders_cancelled_total');
  }

  recordOrdersByStatus(status: string, count: number): void {
    this.gauge('orders_by_status', count, { status });
  }

  recordOrderItemCompleted(recipeName: string, preparationTimeSeconds: number): void {
    this.increment('order_items_completed_total', { recipe_name: recipeName });
    this.increment('order_items_by_recipe', { recipe_name: recipeName });
    this.observe('order_item_preparation_time_seconds', preparationTimeSeconds, { recipe_name: recipeName });
  }

  recordOrderItemFailed(recipeName?: string): void {
    this.increment('order_items_failed_total', { recipe_name: recipeName || 'unknown' });
  }

  recordOrderItemsByStatus(status: string, count: number): void {
    this.gauge('order_items_by_status', count, { status });
  }
