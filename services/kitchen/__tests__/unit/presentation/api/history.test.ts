  ...
  describe('GET /api/history', () => {
    ...
    it('should return 400 if no query parameters', async () => {
      ...
      expect(jsonMock).toHaveBeenCalledWith(
        expect.objectContaining({
          success: false,
          error: expect.stringContaining('orderId or plateId is required'),
        })
      );
    });

    it('should return history for plateId', async () => {
      const history = [
        {
          id: 'h1',
          plateId: 'plate-123',
      ...
      mockStatusHistoryRepository.findByPlateId.mockResolvedValue(history);
      mockReq = { method: 'GET', query: { plateId: 'plate-123' } };

      await handler(mockReq as VercelRequest, mockRes as VercelResponse);

      expect(statusMock).toHaveBeenCalledWith(200);
      expect(mockStatusHistoryRepository.findByPlateId).toHaveBeenCalledWith('plate-123');
      expect(jsonMock).toHaveBeenCalledWith(
        expect.objectContaining({
          success: true,
    ...
    it('should return empty array for no history', async () => {
      mockStatusHistoryRepository.findByPlateId.mockResolvedValue([]);
      mockReq = { method: 'GET', query: { plateId: 'non-existent' } };
    ...
    it('should handle errors gracefully', async () => {
      mockStatusHistoryRepository.findByPlateId.mockRejectedValue(new Error('DB error'));
      mockReq = { method: 'GET', query: { plateId: 'plate-123' } };
